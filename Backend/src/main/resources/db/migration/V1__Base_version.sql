...
create table un_news (id int8 not null, content text, date timestamp, header varchar(255), image bytea, tags varchar(255), test timestamp, author_id int8, primary key (id));
...
alter table task add constraint FKoik9gel2hmb8mug9liwwfll6b foreign key (user_id) references un_user;
alter table task add constraint FKp0gwoaxfvah04va0eqhlhj0oy foreign key (task_description) references task_description;
alter table task_description add constraint FKrpgf0yag4cj4b9mal3k0xi6lw foreign key (user_id) references un_user;
